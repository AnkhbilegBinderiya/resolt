...
const PredictionPoint = ({id, homeId, awayId}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response1 = await fetch(`http://localhost:6969/prediction/nba/${id}?team=${homeId}`);
                if (!response1.ok) {
                ...
                const response2 = await fetch(`http://localhost:6969/prediction/nba/${id}?team=${awayId}`);
                if (!response2.ok) {
                    throw new Error('No data found');
                }
    ...
    return (
        <div className="flex flex-col w-full rounded-lg text-black dark:text-white">
            <Table color={"default"}
            removeWrapper={false}
            isHeaderSticky={true}
