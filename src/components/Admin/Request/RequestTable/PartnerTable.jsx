...
const PartnerTable = () => {
    ...
    useEffect(() => {
        ...
        try {
            const response = await fetch(`http://localhost:6969/admin/crm/partner/request`);
            if (!response.ok) {
  ...
  const saveCustomerData = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:6969/admin/crm/status/request/partner", {
        method: "POST",
        headers: {
    ...
    return (
        <div className="w-full">
          ...
          <TableHeader className="bg-primary">
            ...
            <TableColumn className="bg-black text-white">Bank</TableColumn>
            <TableColumn className="bg-black text-white">Bank Number</TableColumn>
            <TableColumn className="bg-black text-white">Application</TableColumn>
            <TableColumn className="bg-black text-white">App Id</TableColumn>
          ...
          <TableBody>
            {data.map((row) =>
              <TableRow key={row.team_id}>
                ...
                <TableCell className="text-xs md:text-sm">{row.partner_bank}</TableCell>
                <TableCell className="text-xs md:text-sm">{row.partner_number}</TableCell>
                <TableCell className="text-xs md:text-sm">{row.partner_app}</TableCell>
                <TableCell className="text-xs md:text-sm">{row.partner_id}</TableCell>
